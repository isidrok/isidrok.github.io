...
When I fist started looking at computer vision technologies I wanted something that could run in the user device. This has some obvious advantages:

1. It is free.
2. It doesn't require a backend.
3. The data sits with the user.

The first model I came across was Ultralytics [YOLOv8](https://yolov8.com/), a pretty popular, open source, object detection and image segmentation model, which along many other things, supports human keypoint detection with its dedicate pose model. The choice was pretty obvious.
...
In order to load the model we first need the appropriate format, you can find it online but lets go ahead and export it ourselves using [Ultralytics](https://docs.ultralytics.com/quickstart/):
...
And is done, we can now start using the model.
...
Having this into account lets use Tensorflow to create a tensor from an image and have a look at the different tensor shapes we will be dealing with:
...
async function main() {
  ...
  const processedImage = imageTensor
    ...
    .resizeBilinear<Tensor3D>([modelHeight, modelWidth]); // Resize to model input shape
...
- 56: channels, the model outputs 56 values per detection, which includes the person box (x, y, width, height, confidence), and the 17 keypoints (x, y, confidence).
...
<Image src={predictionMisplaced} alt="Original image next to original image with predictions misplaced" />

2. If the image aspect ratio is different from the model input size, the image will be distorted.
<Image src={predictionStretched} alt="Original image next to stretched image with predictions" />

Lets apply letterboxing and scaling transformations to the model output to avoid these issues.
...
The predictions are still relative to the model input size, so we need to scale them back to the original image size. We will also need to account for the added padding.
<Image src={paddedPrediction} alt="Original image next to predictions from padded image" />
...
<Image src={predictionScaledWide} alt="Original wide image next to prediction scaled" />
